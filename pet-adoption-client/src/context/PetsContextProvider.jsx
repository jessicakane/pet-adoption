...
export const PetsContextProvider = ({children}) => {
    ...
    const fetchAllPets = async () => {
        try {
            const response = await axios.get('http://localhost:8080/pets', {withCredentials: true});
    ...
    const searchAllPets = async (searchQuery) => {
      try {
          ...
          const response = await axios.get(`http://localhost:8080/pets/search?search=${searchQuery}`, {withCredentials: true});
          let allPets = (response.data);
          console.log(response.data)
          let availablePets = allPets.filter(pet => pet.adoptionStatus === 'Up for adoption');
          setPets(allPets);
    ...
    const fetchAllPetsNoToken = async () => {
      try {
          const response = await axios.get('http://localhost:8080/pets/allpets');
          let allPets = (response.data);
          setPets(allPets);
          console.log(allPets);
      } catch (error) {
    ...
    const deletePet = async (petId) => {
      try {
        const response = await axios.delete(`http://localhost:8080/pets/${petId}`, {withCredentials: true});
        await fetchAllPets();
      } catch (error) {
    ...
    const fetchPetById = async (petId) => {
      try {
          const response = await axios.get(`http://localhost:8080/pets/${petId}`, {withCredentials: true});
    ...
    const updatePet = async (petId, fieldsToUpdate, id) => {
        try {
            await axios.post(`http://localhost:8080/pets/${petId}`, fieldsToUpdate, {withCredentials: true});
          await fetchAllPets();
