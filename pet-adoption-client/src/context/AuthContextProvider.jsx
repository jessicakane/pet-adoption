...
export const AuthContextProvider = ({children}) => {
    ...
    const isUserSignedIn = async() => {
        try {
            await axios.get('http://localhost:8080/pets', {withCredentials: true})
        } catch (error) {   
    ...
    const fetchUserInfo = async (userId) => {
        try {
            const response = await axios.get(`http://localhost:8080/users/info/${userId}`, {withCredentials: true});
    ...
    const updateUserInfo = async (userInfo, id, setPasswordError) => {
      for (const pair of userInfo.entries()) {
        ...
        try {
                await axios.post(`http://localhost:8080/users/update/${id}`, userInfo, {withCredentials: true});
    ...
    const makeAdmin = async(id) => {
        ...
        try {
            await axios.post(`http://localhost:8080/users/update/${id}`, adminObj, {withCredentials: true});
            return true;
        } catch (error) {
